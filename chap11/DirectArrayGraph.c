...
DirectArrayGraph *createDirectArrayGraph(int nodeCount)
{
    ...
    pReturn = (DirectArrayGraph *) malloc(sizeof(DirectArrayGraph));   //방향 그래프 구주체 자체의 메모리 할당 및 검증
    if (pReturn == NULL) {
        return NULL;
    }

    pReturn->nodeCount = nodeCount;                                         //노드 개수에 대한 멤버 변수 초기화
    pReturn->ppEdge = (int **) malloc(sizeof(int *) * nodeCount);      //'포인터 변수'들의 1차원 배열을 위한 메모리 할당 및 검증
    ...
    for(i = 0; i<nodeCount;i++){
        pReturn ->ppEdge[i] = (int *)malloc(sizeof(int)*nodeCount);    //각각의 줄별로 메모리 할당 및 검증증

        if(pReturn->ppEdge[i] ==NULL){
            return NULL;            //중간에 할당된 메모리 해제 필요!!
        }
        memset(pReturn ->ppEdge[i],0,sizeof(int)*nodeCount);  //각 줄 별로 메모리 초기화
    }
    return pReturn;
}
