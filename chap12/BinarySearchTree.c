...
BinSearchTreeNode *searchBST(BinSearchTree *pBinSearchTree, int key) {
    BinSearchTreeNode *pReturn = NULL;      // 이진 검색 트피가 NULL인지 유효성 점검
...
int getParentNode(BinSearchTreeNode *pCurrentNode, int key,
                  BinSearchTreeNode **ppResult) {
    int ret = 1;
...
void insertNewBinSearchTreeNode(BinSearchTree *pBinSearchTee,
                                BinSearchTreeNode *pParentNode, BinSearchTreeNode *pNewNode) {
    if (pParentNode == NULL) {
        pBinSearchTee->pRootNode = pNewNode;
